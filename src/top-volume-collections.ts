...
const server = new McpServer({
  ...
  version: "0.0.2",
  capabilities: ["top-volume-collections"],
});

// Tool: get-user-collections
server.tool(
  "get-top-volume-collections",
  "Retrieve top volume NFT collections on Magic Eden testnet",
  {
  period: z.string().describe("Magic Eden user address on Monad testnet"),
  },
  async ({ period }) => {
    const url =
    `https://api-mainnet.magiceden.dev/v3/rtp/monad-testnet/collections/trending/v1?period=${period}&limit=50&sortBy=volume&normalizeRoyalties=false&useNonFlaggedFloorAsk=false`
    ...
    try {
      ...
      const data =
        (await res.json()) as { collections: Array<{ name: string; volume: string }> };

      // Sort by volume and build output lines
      const lines = data.collections
        .map(item => `${item.name}: ${item.volume} $MON volume`);
